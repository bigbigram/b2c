...
export class CartComponent implements OnInit {
  ...
  getOffers() {
    this.api
      .get_public("v1/offers/getMyOffers")
      .then(
        (data: any) => {
          console.log(data);
  ...
  getStoreList() {
    ...
    this.api
      .post_private("v1/stores/getStoresData", param)
      .then(
        (data: any) => {
          console.log(data);
          if (data && data.status == 200 && data.data.length) {
            ...
            this.storeIds = [
              ...new Set(this.storeAddress.map((item) => item.uid)),
            ];
  ...
  selectedOffers(item) {
    ...
    if (this.cart.totalPrice >= min) {
      ...
      this.util.errorMessage(
        this.util.translate("Sorry") +
        " " +
        this.util.translate("minimum cart value must be") +
        " " +
        min +
        " " +
        this.util.translate("or equal")
      );
    }
  }
  ...
  deleteAddress(item) {
    Swal.fire({
      ...
      if (data && data.value) {
        ...
        this.util.start();
        this.api
          .post_private("v1/address/deleteMyAddress", param)
          .then(
            (info) => {
              console.log(info);
              this.util.stop();
  ...
  addAddress() {
    ...
    if (typeof google == "object" && typeof google.maps == "object") {
      ...
      geocoder.geocode(
        ...
        (results, status) => {
          ...
          if (status == "OK" && results && results.length) {
            ...
            this.api
              .post_private("v1/address/addNew", param)
              .then(
                (data: any) => {
                  this.util.stop();
      ...
      this.util.errorMessage(
        this.util.translate(
          "Error while fetching google maps... please check your google maps key"
        )
      );
      return false;
    }
      ...
      geocoder.geocode(
        ...
        (results, status) => {
          ...
          if (status == "OK" && results && results.length) {
            ...
            this.util.start();
            this.api.post_private("v1/address/updateMyAddress", param).then(
              (data: any) => {
                this.util.stop();
                if (data && data.status == 200) {
                  this.getAddress();
  ...
  next() {
    ...
    if (this.cart.totalPrice < this.cart.minOrderPrice) {
      ...
      this.util.errorMessage(
        this.util.translate("Minimum order amount must be") +
        text +
        " " +
        this.util.translate("or more")
      );
      return false;
    }
  ...
  stripeWebPay() {
    ...
    this.util.start();
    this.api
      .post_private("v1/orders/create", param)
      .then(
        async (data: any) => {
          console.log(data);
          this.util.stop();
          ...
          if (
            ...
            const grandTotal = this.cart.grandTotal * 100;
            this.cart.clearCart();
            this.cart.clearCart();
            this.cd.detectChanges();
            ...
            this.api
              .post_private("v1/orders/sendMailForOrders", params)
              .then(
                (data: any) => {
                  console.log(data);
                },
            ...
            this.util.start();
            this.api
              .post_private("v1/payments/stripeWebCheckout", param)
              .then(
                (data: any) => {
    ...
    this.api
      ...
      .then(
        (data: any) => {
          ...
          this.util.stop();
          if (data && data.status && data.status == 200 && data.data) {
            const payMethod = this.payments.filter(
              (x) => x.id == this.pay_method
            );
            console.log(payMethod);
        ...
        (data: any) => {
          ...
          if (data && data.status && data.status == 200 && data.data) {
            ...
            var options = {
              ...
              email: this.getEmail(),
              logo:
                this.util && this.util.appLogo
                  ? this.api.mediaURL + this.util.appLogo
                  : "null",
              name: this.getName(),
              app_color:
      ...
      .then(
        (data: any) => {
          ...
          if (
            ...
            this.api
              .post_private("v1/orders/sendMailForOrders", param)
              .then(
                (data: any) => {
                  console.log(data);
                },
  ...
  sendNotification(orderId) {
    ...
    this.api
      .post_private("v1/notification/sendToStore", param)
      .then(
        (data: any) => {
